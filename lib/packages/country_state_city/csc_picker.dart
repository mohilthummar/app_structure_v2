...
class CSCPicker extends StatefulWidget {
  ...
  const CSCPicker({
    ...
    this.boxDecoration,
    this.boxColor,
    this.boxDisabledColor,
    this.spaceBetweenBox,
  ...
  final TextStyle? selectedItemStyle, dropdownHeadingStyle, dropdownItemStyle, labelStyle;
...
class CSCPickerState extends State<CSCPicker> {
  ...
  Future<dynamic> getResponse() async {
    var res = await rootBundle.loadString('lib/packages/country_state_city/assets/country.json');
  ...
  Future<List<String?>> getStates() async {
    ...
    var takeState = response.map((map) => CSCModel.fromJson(map)).where((item) => item.name == _selectedCountry).map((item) => item.state).toList();
  ...
  Future<List<String?>> getCities() async {
    _cities.clear();
    var response = await getResponse();
    var takeCity = response.map((map) => CSCModel.fromJson(map)).where((item) => item.name == _selectedCountry).map((item) => item.state).toList();
  ...
  void _onSelectedCountry(String value) {
    ...
    setState(() {
      ...
      if (value != _selectedCountry) {
        ...
        _selectedCountry = value.substring(6).trim();
        getStates();
      } else {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        countryDropdown(),
        SizedBox(height: widget.spaceBetweenBox ?? 10),
        stateDropdown(),
        SizedBox(height: widget.spaceBetweenBox ?? 10),
        cityDropdown(),
      ],
  ...
  Widget countryDropdown() {
    return DropdownWithSearch(
      ...
      boxDisabledColor: widget.boxDisabledColor,
      items: _country.map((String? dropDownStringItem) => dropDownStringItem).toList(),
      dropdownTitle: widget.countryDropdownLabel,
      dropdownTitleStyle: widget.dropdownHeadingStyle,
      dropdownHintText: widget.countrySearchPlaceholder,
      dropdownItemStyle: widget.dropdownItemStyle,
      searchBarRadius: widget.searchBarRadius,
      dialogRadius: widget.dropdownDialogRadius,
      disabled: _country.isEmpty ? true : false,
      onChanged: (value) {
  ...
  Widget stateDropdown() {
    return DropdownWithSearch(
      ...
      boxColor: widget.boxColor,
      boxDisabledColor: widget.boxDisabledColor,
      items: _states.map((String? dropDownStringItem) => dropDownStringItem).toList(),
      dropdownTitle: widget.stateDropdownLabel,
      dropdownTitleStyle: widget.dropdownHeadingStyle,
      dropdownHintText: widget.stateSearchPlaceholder,
      dropdownItemStyle: widget.dropdownItemStyle,
      searchBarRadius: widget.searchBarRadius,
      dialogRadius: widget.dropdownDialogRadius,
      disabled: _selectedCountry == "Country" ? true : false,
      onChanged: (value) {
        debugPrint("StateChanged $value -- $_selectedState");
  ...
  Widget cityDropdown() {
    return DropdownWithSearch(
      ...
      dropdownTitle: widget.cityDropdownLabel,
      dropdownTitleStyle: widget.dropdownHeadingStyle,
      dropdownHintText: widget.citySearchPlaceholder,
      dropdownItemStyle: widget.dropdownItemStyle,
      searchBarRadius: widget.searchBarRadius,
      dialogRadius: widget.dropdownDialogRadius,
      disabled: _selectedState == "State" ? true : false,
      onChanged: (value) {
        debugPrint("CityChanged $value -- $_selectedCity");
