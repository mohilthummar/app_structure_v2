...
find_project_root() {
  local dir="$PWD"
  while [ "$dir" != "/" ]; do
    if [ -f "$dir/package.json" ] || [ -f "$dir/pyproject.toml" ] || [ -f "$dir/Cargo.toml" ] || [ -f "$dir/go.mod" ] || [ -f "$dir/pubspec.yaml" ] || [ -d "$dir/.git" ]; then
      echo "$dir"
      return
    fi
    dir=$(dirname "$dir")
...
# Dart / Flutter (dart format is standard; requires pubspec.yaml at root).
if [ "$FORMATTED" = false ] && command -v dart >/dev/null 2>&1 && [ -f "$ROOT/pubspec.yaml" ]; then
  case "$EXTENSION" in
    dart)
      dart format "$FILE_PATH" >/dev/null 2>&1 && FORMATTED=true
      ;;
  esac
fi
