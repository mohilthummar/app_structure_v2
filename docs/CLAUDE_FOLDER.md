...
|---|---|---|
| `architecture.md` | Always | Boot order (11 steps in `bootstrap.dart`), per-feature layer contract (`domain/` interface ← `data/` impl ← `presentation/` controller+view+binding), `BaseController` usage, full DI table (permanent vs lazy+fenix), routing + `AnalyticsService.observer`, network stack, UI state (`ViewState` + `StateSwitch`), i18n + theme + telemetry + notifications + deep links + logging. |
| `code-quality.md` | Always | Anti-default rules, design-token discipline (no inline `Color`, raw doubles, inline `TextStyle`, raw user-visible strings), Dart/Flutter naming + suffixes, `package:app_structure/...` imports, code markers, **i18n rule (every user-visible string goes through `I18n.<key>.tr`)**. |
| `testing.md` | Always | Behavior over implementation, run the specific test file, mock the `domain/` repository interface, `mocktail` (no codegen), reset GetX via `Get.testMode = true` + `Get.reset()`, controller tests instantiate directly, repo-impl tests `verify` storage side effects. |
| `error-handling.md` | `paths: lib/features/**/data/**`, `lib/features/**/presentation/**`, `lib/core/network/**`, `lib/core/controllers/**` | Layered errors. Controllers usually call `runGuarded` (from `BaseController`) instead of raw `try/catch`. Never silently swallow (allowed: `AuthRepositoryImpl.logout`, `AuthController.logout`). No `BuildContext` across `await`. Tokens only in `SecureStorageService`. Uncaught errors land in `AppErrorHandler` → `AppLogger.error` → Crashlytics hook. |
| `security.md` | `paths: lib/features/auth/**`, `lib/core/network/**`, `lib/core/storage/**`, `lib/core/services/**`, `lib/core/config/**` | Never commit `.env`. Tokens via `SecureStorageService` only. Validate JSON in `fromJson`. **`NotificationService` and `DeepLinkService` drop non-whitelisted input by default — set `allowedTypes` / `allowedPaths` per project.** Neither service navigates itself — subscribe to their streams. MIME / size / extension checks before file upload. `PermissionService` requests at point-of-use only. Map `DioException` to user-safe strings. Never log token / PII / email in Crashlytics or Analytics. |
